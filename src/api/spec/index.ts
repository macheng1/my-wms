...
const OptionApi = {
   ...
   */
  getOptionPage: (params: QueryOption) =>
    request.get<{
      list: OptionListItem[];
      total: number;
      page: number;
   ...
   * 结构对称，提交后可直接回显
   */
  saveOption: (data: Partial<OptionDetail>) =>
    request.post<OptionDetail>("options/save", data),

  /** * 更新规格值
   * 使用 POST 统一处理，解决 attributeId 必填校验问题
   */
  updateOption: (data: Partial<OptionDetail>) =>
    request.post<OptionDetail>("options/update", data),

  /** * 批量新增规格值 (工业品多规格快速录入专用)
   * values 为字符串数组，后端自动去重插入
   */
  batchSaveOptions: (data: BatchSaveOptions) =>
  ...
  getOptionDetail: (id: string) =>
    request.get<OptionDetail>("options/detail", { params: { id } }),
